...
namespace Channel9
{
	...
	void GC::Semispace::collect()
	{
		...
		//switch pools
		DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "Start GC, old pool %p, new pool %p, %llu used in %llu data blocks\n", m_pools[m_cur_pool], m_pools[!m_cur_pool], m_used, m_data_blocks);
		...
		DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "Scan roots\n");
