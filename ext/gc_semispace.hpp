...
namespace Channel9
{
	...
	private:
		...
		uchar *next(size_t size, uint16_t type)
		{
			...
			if(!m_in_gc)
				DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "Alloc %u type %x ... ", (unsigned)size, type);
			...
			while(1){
				...
				if(data){
					...
					if(!m_in_gc)
						DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "alloc return %p\n", data->m_data);
	...
	public:
		...
		bool mark(tObj **from_ptr)
		{
			...
			if(old->m_pool == m_cur_pool){
				DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "Move %p, type %X already moved\n", from, old->m_type);
				return from;
			}

			if(old->m_forward){
				DEBUG_PRINTF(DEBUG_GC, DEBUG_INFO, "Move %p, type %X => %p\n", from, old->m_type, (*(tObj**)from));
				*from_ptr = *(tObj**)from;
				return true;
			}
